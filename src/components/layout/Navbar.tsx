    ...
    <>
      <nav 
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${
          scrolled || !isHome 
            ? 'bg-white/70 backdrop-blur-md shadow-sm py-3 border-b border-white/50' 
            : 'bg-transparent py-5'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-8 font-semibold text-slate-600 tracking-wide text-xs lg:text-sm">
            <Link to="/" className="hover:text-primary-600 transition-colors uppercase">{t('nav.home')}</Link>
                    ...
                    <div className="glass-heavy rounded-2xl p-2 border border-white/50 shadow-xl">
                      {SERVICES_CONFIG.map((service) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-slate-900 font-bold text-sm uppercase">{t(`services.items.${service.id}.title`)}</p>
                          </div>
                        </Link>
                      ))}
                    </div>
            ...
            <Link to="/#why-us" className="hover:text-primary-600 transition-colors uppercase">{t('nav.whyUs')}</Link>
            ...
            <a href="tel:+9955550199" className="flex items-center gap-2 group hover:text-primary-600 transition-colors">
               <Phone size={16} />
               {t('nav.callToBook')}
            </a>
          </div>
          ...
          >
            <div className="flex flex-col gap-6 text-lg sm:text-xl font-bold text-slate-800 uppercase tracking-wide overflow-y-auto pb-10 custom-scrollbar">
              <div className="flex items-center gap-2 mb-4">
                <button 
                  onClick={() => { toggleLanguage('ka'); setIsMenuOpen(false); }}
                ...
                <button 
                  onClick={() => { toggleLanguage('ru'); setIsMenuOpen(false); }}
                  ...
                  {isMobileServicesOpen && (
                    ...
                    >
                      {SERVICES_CONFIG.map(s => (
                        ...
                        >
                          <span className="text-primary-600">{React.cloneElement(s.icon as React.ReactElement, { size: 16 })}</span>
              ...
              <Link to="/#why-us" onClick={() => setIsMenuOpen(false)}>{t('nav.whyUs')}</Link>
              <a 
                href="tel:+9955550199" 
