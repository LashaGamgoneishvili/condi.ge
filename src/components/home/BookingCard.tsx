          ...
          <div className="glass rounded-[2.4rem] p-8 sm:p-16 md:p-24 overflow-hidden relative shadow-inner bg-white/40 backdrop-blur-2xl text-center border-white/80">
            ...
            >
              ...
              <h2 className="font-display text-4xl sm:text-5xl md:text-7xl font-black mb-8 tracking-tighter text-slate-900 uppercase leading-[0.85]">
                {t('booking.title').split(' ').map((word, i) => (
                  <span key={i} className={i === 2 || i === 5 ? 'text-primary-600 block' : ''}>{word} </span>
                ))}
              </h2>
            ...
            <div className="flex flex-col md:flex-row justify-center gap-6 relative z-10">
              <a 
                href="tel:+9955550199" 
                className="flex items-center justify-between gap-6 px-8 py-6 bg-primary-600 text-white rounded-3xl hover:bg-primary-700 hover:scale-[1.02] transition-all shadow-xl shadow-primary-500/20 group w-full md:w-auto min-w-[280px]"
              >
                <div className="flex flex-col items-start origin-left transition-transform">
                  ...
                  <span className="text-xl sm:text-2xl font-black tracking-tighter">+995 555 01-99</span>
                </div>
              ...
              <a 
                ...
                className="flex items-center justify-between gap-6 px-8 py-6 bg-white border-2 border-green-500/20 text-green-600 rounded-3xl hover:bg-green-50 hover:border-green-500/40 hover:scale-[1.02] transition-all shadow-xl shadow-green-500/5 group w-full md:w-auto min-w-[280px]"
              >
                <div className="flex flex-col items-start origin-left transition-transform">
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-1 flex items-center gap-2">
                    <MessageCircle size={10} /> Secure Chat
                  </span>
                  <span className="text-xl sm:text-2xl font-black tracking-tighter uppercase">{t('booking.whatsapp')}</span>
                </div>
            ...
            <div className="mt-12 flex items-center justify-center gap-6 opacity-40">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">{t('booking.serving')}</p>
              <div className="h-px w-12 bg-slate-400" />
            </div>
          </div>
        </div>
